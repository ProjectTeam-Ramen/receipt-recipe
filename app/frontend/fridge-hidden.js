...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let current = "used";
    tabUsed.addEventListener("click", () => setTab("used"));
    tabDeleted.addEventListener("click", () => setTab("deleted"));

    function setTab(val) {
        current = val;
        tabUsed.classList.toggle("active", val === "used");
        tabDeleted.classList.toggle("active", val === "deleted");
        loadFoods();
    }
    ...
    async function loadFoods() {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/ingredients?status=${current}`, {
            ...
            if (!items.length) {
                foodList.innerHTML = `<tr><td colspan="5">該当する非表示アイテムはありません。</td></tr>`;
                return;
            }
