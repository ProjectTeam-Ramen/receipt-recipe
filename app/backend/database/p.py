...
class Food(Base):
    ...
    is_trackable = Column(Boolean, nullable=False, default=True) # TINYINT(1) は Boolean で
...
class UserFoodSchema(BaseModel):
    ...
    class Config:
        orm_mode = True # SQLAlchemyモデルから自動変換する
    ...
    for recipe in all_recipes:
        ...
        for req_ingredient in recipe.recipe_foods:
            ...
            if not food_master.is_trackable:
                ingredients_detail_list.append(ProposalIngredientSchema(
            ...
            in_stock_g = fridge_stock.get(req_ingredient.food_id, 0.0) # 冷蔵庫にあればg、なければ0
