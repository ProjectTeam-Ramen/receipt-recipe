...
class RecipeDataSource:
    ...
    def load_and_vectorize_recipes(self) -> List[Recipe]:
        ...
        raw_recipes_data = [
            ...
            {'id': 5, 'name': 'エビマヨ', 'req': {'エビ': 150.0, 'マヨネーズ': 50.0, '片栗粉': 20.0}, 
             'time': 20, 'cal': 400, 'feats': {'中華': 1, '副菜': 1, '魚介類': 1, '甘味': 1, '揚げ物': 1}},
             
            # 6. グリーンサラダ (洋食, 副菜, ベジタリアン, 酸味, 生食は無し)
            {'id': 6, 'name': 'グリーンサラダ', 'req': {'レタス': 200.0, 'トマト': 100.0, '酢': 10.0}, 
             'time': 10, 'cal': 150, 'feats': {'洋食': 1, '副菜': 1, 'ベジタリアン': 1, '酸味': 1}},
             
            # 7. タコライス (複合, 主菜, 炒め物)
            {'id': 7, 'name': 'タコライス', 'req': {'合いびき肉': 200.0, 'ご飯': 300.0, 'チーズ': 50.0}, 
             'time': 30, 'cal': 600, 'feats': {'複合': 1, '主菜': 1, '肉類': 1, '炒め物': 1}}, # 合いびき肉は肉類、チーズは複合ではないが、肉と野菜の複合として扱う
             
            # 8. キャベツのスープ (洋食, 汁物, ベジタリアン, 煮込み)
            {'id': 8, 'name': 'キャベツのスープ', 'req': {'キャベツ': 300.0, 'ブイヨン': 10.0, '水': 500.0}, 
             'time': 45, 'cal': 100, 'feats': {'洋食': 1, '汁物': 1, 'ベジタリアン': 1, '煮込み': 1}},
        ]
