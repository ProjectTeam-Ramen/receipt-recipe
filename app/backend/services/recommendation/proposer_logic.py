...
class RecipeProposer:
    def __init__(
        ...
        self.inventory_dict = {
            ing.name: (ing.quantity, ing.expiration_date) for ing in user_inventory
        }
    ...
    def _get_expiration_boost_factor(self, recipe: Recipe) -> float:
        ...
        for name in recipe.required_qty.keys():
            ...
            if (
                ...
                and (
                    expiry_date <= (today + timedelta(days=self.EXPIRATION_BOOST_DAYS))
                )
            ):
    ...
    def propose(self, params: UserParameters) -> List[Dict]:
        ...
        for recipe in self.all_recipes:
            ...
            # 2.1 アレルギーチェック
            is_allergic = any(
                ing in params.allergies for ing in recipe.required_qty.keys()
            )
            if is_allergic:
                continue
            ...
            # 4. 最終提案スコアの算出
            final_score_base = (coverage_score * self.WEIGHT_INVENTORY) + (
